...
type PwCheck = {
  ...
  checks: {  // detailed boolean checks for visual checklist
    len12: boolean;
    lower: boolean;
    upper: boolean;
    digit: boolean;
...
function evaluatePassword(pw: string, email: string, fullName: string): PwCheck {
  ...
  const containsName = nameParts.some((p) => p.length >= 3 && pwLower.includes(p));
  const containsEmailLocal = emailLocal.length >= 3 && pwLower.includes(emailLocal);
  const noNameOrEmail = !(containsName || containsEmailLocal);

  // Avoid common weak sequences
  const commonSeqs = ['12345','123456','qwerty','password','abc123','1111','0000','letmein'];
  ...
  const labels: PwCheck['label'][] = ['Very weak','Weak','Fair','Strong','Very strong'];
  ...
  if (!noNameOrEmail) messages.push('Avoid using your name or email in the password.');
  if (!noCommonSeq) messages.push('Avoid common sequences like "123456" or "password".');
...
export default function SignupPage() {
  ...
  useEffect(() => {
    ...
    setRole(roleParam);
    setOrgAction(orgParam === 'create' ? 'create' : orgParam === 'join' ? 'join' : null);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const orgName   = orgAction === 'create' ? (e.currentTarget.orgName as HTMLInputElement | undefined)?.value : undefined;
    const inviteCode= orgAction === 'join'   ? (e.currentTarget.invite  as HTMLInputElement | undefined)?.value : undefined;
    ...
    if (pwCheck.score < 3) {
      setError('Password is not strong enough. Please improve it before continuing.');
      return;
    }
    if (password !== confirm) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F3E9D9] flex flex-col items-center justify-center px-4">
      ...
      <div className="absolute left-8 top-8">
        <Image src="/logo-name.png" alt="Scheduling of Care" width={210} height={64} priority />
      </div>
      ...
      {success && (
        <div role="alert" className="fixed top-0 left-0 w-full bg-[#DCF4D9] text-[#1B4B1B] px-6 py-4 text-center font-semibold shadow-md z-50">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg space-y-8 text-black">
        ...
        {(role === 'management' || role === 'carer') && orgAction && (
          <div className="flex flex-col gap-2">
            ...
            {orgAction === 'join' && (role === 'carer' || role === 'management') && (
              <>
        ...
        {/* Full Name */}
        <div className="flex flex-col gap-2">
          <label htmlFor="userName" className="text-[20px] font-medium">Enter Full Name</label>
          <input id="userName" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)}
            className="w-full rounded-md border border-[#6E1B1B] bg-white px-4 py-2.5 text-lg" required />
        </div>

        {/* Email */}
        <div className="flex flex-col gap-2">
          <label htmlFor="email" className="text-[20px] font-medium">Enter Email</label>
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="password" className="text-[20px] font-medium">Create Password</label>
          <div className="relative w-full">
            <input
              id="password"
              type={showPw ? 'text' : 'password'}
              value={password}
              onChange={(e) => { if (!pwTouched) setPwTouched(true); setPassword(e.target.value); }}
              ...
              <div className="mt-2">
                <div className="w-full h-2 rounded bg-[#EADBC6] overflow-hidden">
                  <div className="h-2 rounded transition-all"
                    style={{ width: `${(pwCheck.score / 4) * 100}%`, backgroundColor: pwCheck.color }} />
                </div>
                <div className="mt-1 text-sm" style={{ color: pwCheck.color }}>
                  Strength: <span className="font-semibold">{pwCheck.label}</span>
                </div>
              </div>

              {/* Checklist */}
              <ul className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                <li className="flex items-center gap-2"><Check ok={pwCheck.checks.len12} /> At least 12 characters</li>
                <li className="flex items-center gap-2"><Check ok={pwCheck.checks.lower && pwCheck.checks.upper} /> Upper & lower case</li>
                <li className="flex items-center gap-2"><Check ok={pwCheck.checks.digit} /> At least one number</li>
                <li className="flex items-center gap-2"><Check ok={pwCheck.checks.symbol} /> At least one symbol</li>
                <li className="flex items-center gap-2"><Check ok={pwCheck.checks.noNameOrEmail} /> No name/email</li>
                <li className="flex items-center gap-2"><Check ok={pwCheck.checks.noCommonSeq} /> No common sequences</li>
              </ul>

              {/* Suggestions */}
              {pwCheck.messages.length > 0 && (
                <div className="mt-2 bg-[#FFF3CD] text-[#5C3C00] px-3 py-2 rounded text-sm">
                  <div className="font-semibold mb-1">Tips to improve your password:</div>
                  <ul className="list-disc pl-5 space-y-1">
                    {pwCheck.messages.map((m, i) => (<li key={i}>{m}</li>))}
                  </ul>
                </div>
              )}
            </>
        ...
        {/* Confirm password */}
        <div className="flex flex-col gap-2">
          <label htmlFor="confirm" className="text-[20px] font-medium">Retype Password</label>
          <input
            id="confirm"
            type={showPw ? 'text' : 'password'}
            value={confirm}
            onChange={(e) => { if (!confirmTouched) setConfirmTouched(true); setConfirm(e.target.value); }}
            onFocus={() => setConfirmTouched(true)}
            className="w-full rounded-md border border-[#6E1B1B] bg-white px-4 py-2.5 text-lg"
            required
          />
          {mismatchVisible && (
            <div className="text-red-700 bg-red-100 px-3 py-2 rounded text-sm">Passwords do not match.</div>
          )}
        </div>

        {/* Error messages */}
        {error && <div className="text-red-700 bg-red-100 px-4 py-2 rounded">{error}</div>}
        {userExists && (
          <div className="bg-[#DFC9A9] px-4 py-2 rounded">
            Account already exists. Try a new email or{' '}
            <Link href={`/?email=${encodeURIComponent(email)}`} className="underline font-bold text-[#4A0A0A]">
              login
            </Link>
          </div>
        )}
        ...
        <div className="flex flex-col items-center pt-2">
          <button
            ...
            className={`rounded-full text-white text-xl font-semibold px-10 py-3 transition ${
              canSubmit ? 'bg-[#4A0A0A] hover:opacity-95' : 'bg-[#a78b7a] cursor-not-allowed'
            }`}
          >
            Sign Up
          </button>
          {!canSubmit && pwTouched && (
            <div className="mt-2 text-sm text-[#4A0A0A]">
              {mismatchVisible ? 'Please make sure both passwords match.' : 'Password must be at least Strong to continue.'}
            </div>
          )}
        </div>
        ...
        <p className="text-center text-lg mt-4">
          ...
          <Link href="/role" className="underline font-bold">Role Selection</Link>
        </p>
      </form>
