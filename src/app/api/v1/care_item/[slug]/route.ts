...
export async function PUT(req: Request, ctx:{params: Promise<{slug: string}>}) {
    ...
    const exist = await CareItem.findOne({slug: slug.toLowerCase()}).lean<CareItemDoc | null>();
    if(!exist) return errorJson("Task not found", 404);

    // Find clientId to be updated
    const clientIdStr = body.clientId ?? (exist.clientId ? String(exist.clientId) : null);
    if(!clientIdStr || !Types.ObjectId.isValid(clientIdStr)) return errorJson("clientId must be a valid ObjectId -- care_item/[slug]/route.ts",);
    ...
    if(typeof body.category === "string" && body.category.trim()) {
        const c = await findOrCreateNewCategory({clientId, input: body.category.trim()});
    ...
    if (typeof body.label === "string" && body.label.trim()) setPayLoad.label = body.label.trim(); 
    if (typeof body.status === "string" && body.status.trim()) setPayLoad.status = body.status.trim(); 
    ...
    if (typeof body.clientName === "string" && body.clientName.trim()) setPayLoad.clientName = body.clientName.trim();
    if (normalised.dateFrom !== undefined) setPayLoad.dateFrom = normalised.dateFrom as string; 
    if (normalised.dateTo !== undefined) setPayLoad.dateTo = normalised.dateTo as string;

    if (normalised.frequencyCount !== undefined) setPayLoad.frequencyCount = normalised.frequencyCount; 
    if (normalised.frequencyUnit !== undefined) setPayLoad.frequencyUnit = normalised.frequencyUnit; 
    if (normalised.frequencyDays !== undefined) setPayLoad.frequencyDays = normalised.frequencyDays; 
    if (normalised.frequency !== undefined) setPayLoad.frequency = normalised.frequency; 
    if (normalised.lastDone !== undefined) setPayLoad.lastDone = normalised.lastDone;
