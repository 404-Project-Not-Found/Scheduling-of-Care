...
export async function GET(req: Request) {
  ...
  if (clientIdStr) {
    ...
    const list = await Category.find({ ...baseFilter, clientId }).sort({ name: 1 }).limit(200).lean();
...
export async function POST(req: Request) {
    ...
    const clientIdStr = typeof body.clientId === "string" ? body.clientId.trim() : "";

    if(!input) return NextResponse.json({error: "Category name required"}, {status: 422});
    if(!isObjectIdString(clientIdStr)) return NextResponse.json({error: "clientId must be a valid ObjectID"}, {status: 422});
