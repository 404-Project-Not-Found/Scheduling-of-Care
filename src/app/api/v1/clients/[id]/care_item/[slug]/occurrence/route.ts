...
export async function GET(
  ...
  const doc = await Occurrence.findOne(
    ...
    { careItemSlug: 1, clientId: 1, date: 1, status: 1, files: 1, comments: 1, _id: 0 }
  ).lean<Pick<IOccurrence, 'careItemSlug' | 'date' | 'status' | 'files' | 'comments'>>();

  if (!doc) return NextResponse.json({ error: 'Occurrence not found' }, { status: 404 });
