...
export async function GET(req: Request) {
  ...
  if(!clientIdrStr || !Types.ObjectId.isValid(clientIdrStr)) return errorJson("clientId must be valid ObjectId -- task_catalog/route.ts", 422);
  ...
  if(categoryIdStr) {
    if(!Types.ObjectId.isValid(categoryIdStr)) return errorJson("categoryId must be valid ObjectId -- task_catalog/route.ts", 422);
    catId = new Types.ObjectId(categoryIdStr);

    //fetch name to display
    const cat = await Category.findOne({ _id: catId, name: clientId}).select({name: 1}).lean<CategoryLean | null>();
    if(cat?.name) displayName = cat.name;
  } 
  else if(categorySlug) {
    const cat = await Category.findOne({ clientId, slug: categorySlug.toLowerCase()}).select({_id: 1, name: 1}).lean<CategoryLean | null>();
    if(!cat) return errorJson("Category not found for given slug and clientId", 404);
  ...
  else {filter.category = categoryName}
