 ...
 * - Right side: renders ONLY the selected page.
 * - Left sidebar: lists pages from the SAME ROLE as the active page.
 * - "Current page" badge is pinned on the active entry.
 * - Sidebar auto-scrolls to keep the current entry in view.
 * - Supports whitelist (allowedPageKeys) and anchor scroll (sectionId).
 * - Subtitles are intentionally NOT rendered.
 */
...
export type HelpOpenOptions = {
  /** Optional whitelist: when provided, only these pages appear in the panel (after role filter). */
...
function HelpPanelOverlay({
  ...
  // Determine role from the initially requested page
  const initialRole = getRoleFromKey(pageKey);

  // Build the list of visible keys: same role + whitelist (if any)
  ...
  // Fallback if requested page not visible
  ...
  // Sidebar auto-scroll to keep current entry in view
  const sidebarRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const el = document.getElementById(`toc-${activeKey}`);
    if (el && sidebarRef.current) {
      // Use nearest so it won't jump if already visible
  ...
  const roleLabel =
    initialRole === 'family' ? 'Family / POA' : initialRole === 'carer' ? 'Carer' : initialRole === 'management' ? 'Management' : 'Help';
  ...
  return createPortal(
    <>
      ...
      >
        ...
        >
          {/* LEFT: page list (whitelist-aware). Each entry anchors to its own divider on the right. */}
            ...
            >
              <ul className="flex flex-col gap-5">
                {visibleKeys.map((key) => {
                  ...
                  const isCurrent = key === activeKey; // pin the badge to the ACTIVE page entry
                  return (
                    <li key={key}>
                      <button
                        id={`toc-${key}`} 
                        ...
                        onClick={() => {
                          setActiveKey(key);
                          // after switching page, jump to its divider (and pin highlight there)
          ...
          {/* RIGHT: single page content + top divider for stable anchors */}
          <div ref={contentRef} className="space-y-8 h-full overflow-y-auto pr-1">
            {/* Page divider (non-collapsible), provides a stable anchor for the sidebar */}
            ...
            {/* Actual sections of the active page */}
 ...
 * - Divider: handled above (one per page). Here we render only real sections.
 * - Normal section: collapsible Q/A. Subtitles are intentionally NOT rendered.
 * ------------------------------------------------------------------- */
function FAQSectionBlock({ section, color }: { section: FAQSection; color: string }) {
  // Never render page dividers here; those are injected in the overlay above.
